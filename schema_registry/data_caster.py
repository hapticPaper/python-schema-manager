...
class DataCaster:
    ...
    def __init__(self, schema: TableSchema, sql_engine: SQLEngine = SQLEngine.BIGQUERY):
        ...
        if HAS_STRUCTLOG:
            ...
            logger.info(f"DataCaster initialized for table {schema.name} with {len(schema.columns)} columns (Engine: {sql_engine.value})")
        ...
        for column in self.schema.columns:
            ...
            try:
                ...
                if comp_str.startswith('strftime(') and comp_str.endswith(')'):
                    ...
                    if HAS_STRUCTLOG:
                        logger.warning("Unknown computation format", 
                                     column_name=column.name, 
                ...
                if HAS_STRUCTLOG:
                    logger.error("Error computing field", 
                               column_name=column.name, 
    ...
    def cast_dataframe(self, df: pd.DataFrame, inplace: bool = True) -> pd.DataFrame:
        ...
        for column in self.schema.columns:
            ...
            if handler:
                try:
                    # Skip if column is not in dataframe (will be handled by null fill logic if we moved it, but here we assume it sits there)
                    if column.name not in casted_df.columns:
                         continue
                    ...
                    if column.type in (SQLType.TIMESTAMP, SQLType.DATETIME):
                        ...
                        if target_dtype and str(current_dtype) == target_dtype:
                             # Already correct
                             mapped = True
                        else:
                            mapped = False
